...
class MainWin(QMainWindow, form_class):
    ...
    def fillCoinData(
        ...
        print(self.ticker)
        if trade_price < 1:
            self.trade_price.setText(f"{trade_price:,.5f}원")
            ...
            if self.alarmFlag == 0:
                ...
                if self.alarmPrice1.text():
                    ...
                    if trade_price >= sell_price:
                        print("매도가격 도달! 매도하세요!")
                        self.telegram_message(f"Current price: {trade_price:,}\nTarget sell price({sell_price:,}) reached.")
                        self.alarmFlag = 1

                if self.alarmPrice2.text():
                    buy_price = float(self.alarmPrice2.text())  # 사용자가 입력한 매수 목표 가격

                    # 현재 코인 가격이 사용자가 설정해 높은 매수 가격보다 낮아지면 매도 알람
                    if trade_price <= buy_price:
                        print("매수가격 도달! 매수하세요!")
                        self.telegram_message(f"Current price: {trade_price:,}\nTarget buy price({buy_price:,}) reached.")
                        self.alarmFlag = 1
        else:
            pass
